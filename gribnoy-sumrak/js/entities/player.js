...
export class Player {
  ...
  update(input,w,h,dt,enemies){
    ...
    this.x=clamp(this.x,this.radius,w-this.radius); this.y=clamp(this.y,this.radius,h-this.radius);
    // Угол: мышь на ПК, авто-прицел на мобильных
    ...
    if(this.regen>0 && this.hp<this.maxHp && Math.random()<0.02) this.hp+=this.regen;
    this.sporeLevel+=CONFIG.player.sporeGrowth*dt; this.sporeLevel=clamp(this.sporeLevel,0,100);
    if(this.sporeLevel>=75) this.hp-=CONFIG.sporeSystem.effects.critical.hpDrain*dt;
    this.animTimer++;
    if(this.animTimer>=this.animSpeed){ this.animTimer=0; this.animFrame=(this.animFrame+1)%CONFIG.player.spriteCols; }
  }
  tryShoot(){
    if(this.attackCooldown<=0){
      this.attackCooldown=this.attackRate;
      return new Projectile(this.x+Math.cos(this.angle)*(this.radius+6),this.y+Math.sin(this.angle)*(this.radius+6),this.angle,this.damage,this.projectileType);
    }
    return null;
  }
  takeDamage(a){
  ...
  addXp(a){
    ...
    while(this.xp>=this.xpToNext){ this.xp-=this.xpToNext; this.level++; this.xpToNext=Math.floor(this.xpToNext*1.35)+5; this.damage+=2; this.maxHp+=8; this.hp=Math.min(this.hp+15,this.maxHp); leveledUp=true; }
    return leveledUp;
  }
  ...
  draw(renderer){
    ...
    if(img){
      ...
      renderer.drawSpriteSheet(img,this.x,this.y,CONFIG.player.spriteFrameW,CONFIG.player.spriteFrameH,this.animFrame,row,CONFIG.player.spriteDisplaySize,this.angle);
    } else {
      ...
      for(let side=-1;side<=1;side+=2){
        const ex=this.x+Math.cos(this.angle+side*0.45)*this.radius*0.45, ey=this.y+Math.sin(this.angle+side*0.45)*this.radius*0.45;
        renderer.drawCircle(ex,ey,4,"#00d4aa"); renderer.drawCircle(ex+Math.cos(this.angle)*1.5,ey+Math.sin(this.angle)*1.5,2,"#000");
      }
      ...
      renderer.drawGlowCircle(this.x-8,this.y+10,3,"#39ff14",6); renderer.drawCircle(this.x-8,this.y+10,3,"#39ff14");
      renderer.drawGlowCircle(this.x+8,this.y+10,3,"#c4a000",6); renderer.drawCircle(this.x+8,this.y+10,3,"#c4a000");
    }
    if(this.shieldActive&&this.shieldTimer>0){ renderer.ctx.strokeStyle="rgba(0,212,170,0.6)"; renderer.ctx.lineWidth=2; renderer.ctx.beginPath(); renderer.ctx.arc(this.x,this.y,this.radius+8,0,Math.PI*2); renderer.ctx.stroke(); }
