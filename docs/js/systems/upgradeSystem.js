...
export class UpgradeSystem {
  ...
  showMenu(cards,player){
    ...
    cards.forEach((card,i)=>{
      ...
      // КАРТОЧКА ТОЛЬКО СООБЩАЕТ О ВЫБОРЕ. Прячет меню тот, кто знает, есть
      // ли ещё что выбирать (main: уровни теперь копятся и берутся пачкой).
      //
      // Раньше строка `menu.classList.add("hidden")` стояла здесь, сразу
      // после отправки события, — и это ломалось ровно на очереди: обработчик
      // события успевал показать СЛЕДУЮЩУЮ тройку, а эта строка тут же её
      // прятала. Игра оставалась на паузе перед пустым экраном.
      //
      // Заодно чинится старая мелочь: событие, посланное из кода (боты,
      // замеры), оставляло разметку меню висеть на экране — потому что
      // прятала её не игра, а обработчик мыши.
      div.onclick=()=>{
