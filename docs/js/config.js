...
export const CONFIG = {
  ...
  player: {
    ...
    radius: 16, speed: 3.0, maxHp: 100, damage: 10, attackRate: 30,
    ...
    // Раньше ряд листа выбирался по УГЛУ ПРИЦЕЛА, а прицел висит на мыши.
    // Стоило повести курсором вдоль персонажа — и он дёргано разворачивался
    // туда-сюда по несколько раз в секунду, потому что граница между рядами
    // проходит ровно по диагоналям и мышь всё время её пересекала.
    //
    // Теперь при ходьбе персонаж смотрит ТУДА, КУДА ИДЁТ (как и положено
    // спрайту ходьбы — иначе он шагает боком), а прицел решает только когда
    // игрок стоит на месте. Плюс два предохранителя от дёрганья:
  ...
  enemies: {
    ...
    // 34 → 28: врагов на поле стало больше, но и цена каждого касания должна
    // ощущаться, иначе толпа безопаснее одиночки.
    touchInterval: 32,
    ...
    types: {
      ...
      fruit_body: {
        name: "Плодовое Тело", hp: 70, speed: 0.5, radius: 22, damage: 0, xpReward: 15,
  ...
  spawn: {
    ...
    aliveBase: 9, aliveMax: 36, aliveRampTime: 260,
    ...
    baseHp: 1.3, baseDamage: 1.1,
    hpPerMin: 0.26, dmgPerMin: 0.12,
    ...
    // Пороги сдвинуты ближе к старту: до 80-й секунды на арене был ровно один
    // тип врага, и первые полторы минуты забега ничем не отличались друг от
    // друга.
    unlock: { mushroom_wolf: 40, spore_bat: 95, fruit_body: 130, mycelium_tentacle: 190 }
  },
  ...
  loot: {
    ...
    // Расходники стали реже: врагов на поле втрое больше, и при прежних
    // шансах антидот с зельем сыпались непрерывно — заражение и здоровье
    // переставали быть ресурсом, за которым надо следить.
