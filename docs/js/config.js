...
export const CONFIG = {
  ...
  waves: {
    ...
    // РОСТ СЛОЖНОСТИ. Раньше от волны к волне менялось только КОЛИЧЕСТВО
    // врагов: их характеристики оставались стартовыми, поэтому прокачанный
    // игрок к десятой волне сносил всех с одного попадания и игра умирала.
    // Теперь враги тоже растут — по волне, а не по времени, чтобы отсидеться
    // в углу было нельзя.
    hpPerWave: 0.11,        // +11% HP за волну (умножается, не складывается)
    dmgPerWave: 0.05,
    speedPerWave: 0.012, speedCapMult: 1.4,
    bossHpPerWave: 0.10,
    // Волна не ждёт, пока поле опустеет полностью: добить последнего волка на
    // другом конце арены — это не геймплей, а пауза. Идём дальше, когда
    // остались единицы.
    nextWaveWhenLeft: 2
  },
