...
export class Renderer {
  ...
  drawProp(key,x,y,w,h,opts={}){
    ...
    const ctx=this.ctx;
    ctx.save();
    // ТЕНИ ПОД ДЕКОРАЦИЕЙ БОЛЬШЕ НЕТ ПО УМОЛЧАНИЮ, и это вторая попытка на
    // одних граблях. Сначала она была резким чёрным эллипсом шире ствола;
    // её растушевали и сузили — и жалоба вернулась дословно той же: «под
    // камнем тень, из-за неё ощущение, что камень летает».
    //
    // Причина, по которой правка размера не помогала: у существ тень работает
    // (они ходят, и пятно под ними показывает, где они стоят), а декорация
    // НЕПОДВИЖНА. Под неподвижным объектом тень не сообщает ничего нового, зато
    // добавляет к его силуэту второе пятно другой формы — и глаз читает их как
    // два разных предмета, один над другим. Отсюда и «летает».
    //
    // Тень осталась опцией: `shadow: true` в записи декорации (CONFIG.map.props)
    // вернёт её конкретному объекту, если такой найдётся. Сейчас её не просит
    // никто, и это не недосмотр.
    if(opts.shadow&&!opts.flat){
