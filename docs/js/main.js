...
function startRun(){
  ...
  setTimeout(()=>{
    loader.warmSounds();
    // Вторая тема забега приходит именно здесь, уже посреди боя. Пока она
    // была одна, первая играла ПО КРУГУ — то есть сама бы никогда не
    // кончилась и очередь бы не сдвинулась. Снимаем зацикливание.
    // Задержка на догрузку: warmSounds только просит браузер начать качать.
    setTimeout(()=>audio.refreshPlaylist(),4000);
  },(CONFIG.assets.warmDelay??8)*1000);
