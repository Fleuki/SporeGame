...
**ЛИСТ СДЕЛАН.** Пришёл 1696x2528, сеткой 4x6 — и на нём вылезли сразу три
грабли, каждая из которых стоит того, чтобы её знать.

1. **Шаг сетки не равен файлу, делённому на сетку.** 2528/6 = 421.3, а
   настоящий шаг — 424: генератор обрезал снизу 16 пикселей. По делению
   разметка уезжала на полкадра уже к третьему ряду. Шаг виден по тому, где
   начинается содержимое каждого ряда, и задаётся руками: `--cell=424`.
2. **Ключ снимался не тем режимом, и босс позеленел.** Режим `saturated`
   исходит из того, что в рисунке есть хотя бы один тёмный канал; серый камень
   (128,128,128) по этой мерке — наполовину магента, и после вычитания фона от
   него оставался ярко-зелёный. Правильный режим для серо-бледного рисунка —
   `pale`: он меряет ПЕРЕКОС в сторону ключа, а у серого перекоса нет.
   Признак беды виден сразу в отчёте скрипта: 35% полупрозрачных пикселей
   против 7% у правильного режима.
3. **Ряды 2, 3 неотличимы от первого, а ряд 5 внутри себя разный** — в одной
   клетке пролом купола, в соседней целый. На таком ряду босс мигал бы между
   целым и пробитым прямо в бою, ведь колонки крутятся как дыхание. Взяты
   ряды **1, 4, 4, 6**: целый → горящее ядро → оно же → развалины. Дублировать
   ряд не стыдно: повреждение читается уже на четверти снятого HP, и ни одна
   фаза не мигает.

Итоговая команда, ей же перебирать при перегенерации:

```
python3 tools/cut_key.py assets-raw/bosses/spore_hive_src.png /tmp/hive.png --mode pale
node tools/pick_rows.mjs --in=/tmp/hive.png --out=assets-raw/bosses/spore_hive.png \
     --grid=4x6 --cell=424 --rows=1,4,4,6 --size=352
...
### Исходник — это файл из генератора, а не то, что дошло до телефона

Замер по первой присланной картинке, потому что на глаз этого не видно:

| | что пришло | что нужно |
|---|---|---|
| формат | JPEG | PNG |
| размер | 274x281 | от 1024 |
| пиксель рисунка | 3 экранных пикселя | 16 |
| уникальных цветов | **11794** | 16–32 |
| фон | шахматка, ЗАПЕЧЁННАЯ в пиксели | ровная магента |

Картинка была хорошая: фигура в полный рост, силуэт читается, снаряжение на
месте. Не пережила она дорогу — «поделиться» с телефона пересжимает PNG в
JPEG, а превью в чате ужимает 1024 до трёхсот пикселей.

**Почему это не чинится позже.** JPEG размывает границу каждого пикселя
рисунка в облачко близких оттенков — на глаз незаметно, а квантование в
палитру превращает это облачко в соль с перцем: ровный фиолетовый плащ
рассыпается на серый, бурый и лиловый вперемешку, потому что каждый пиксель
выбирает свою ближайшую ступень. Прогнали — так и вышло: 16 цветов палитры,
разбросанных как шум. Светящиеся линзы при этом просто погасли: бирюзу
размазало по контуру, и от неё не осталось ничего яркого.

**Размер тут важнее формата.** При 16 экранных пикселях на пиксель рисунка
звон JPEG остаётся внутри блока и усредняется обратно в один чистый цвет. При
трёх — звон И ЕСТЬ весь блок.

**Шахматка вместо прозрачности — отдельная беда**, ровно та, о которой
написано в пункте 2 этого файла. Здесь она была нарисована пикселями: клетка
25 пикселей, два серых оттенка. Выбить её удалось только заливкой от краёв (по
цвету нельзя — те же серые есть на самом противогазе), и после этого по
контуру остался звон, который пришлось счищать отдельно. Всё это не нужно
вовсе, если просить фон МАГЕНТОЙ: у неё в игре нет ни одного близкого цвета,
и снимается она одной командой.

Правило: **скачивать из генератора кнопкой «Скачать», а не сохранять из
превью и не пересылать через телефон.** Проверять исходник по трём числам —
формат, сторона, число цветов, — и только потом класть в `assets-raw`.
