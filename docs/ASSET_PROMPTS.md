...
**Сделаны пятеро.** Осталось одно — **Плодовое Тело**, и оно единственное,
что у модели пока не выходит: широкий плоский купол на тонких корнях искажается
между кадрами сильнее всех. Причина, скорее всего, в референсе: у него кадр 82
пикселя, и увеличение до 512 даёт нецелый множитель 6.24 — пиксельная сетка
в референсе получается кривой. Следующую попытку делать с референсом 656
(82x8), а не 512.

Ещё одно наблюдение оттуда же: **край кадра может обрезать облако прямой
линией.** У Трубача во втором кадре модель увела дым за край картинки, и после
нарезки получился прямоугольный срез — глаз читает его как ошибку. Лечится не
перегенерацией: альфа у края гасится рампой с шумом, и срез становится рваным,
то есть читается как граница облака. Низ при этом трогать нельзя — там существо
стоит на земле.

### Что показал первый готовый лист (Грибной Волк)

Сделан, лежит в игре. Три замечания из практики, они дороже промпта:

* **Перегородки модель нарисовала СЕРЫМИ, а не магентовыми.** Просьба в
  промпте не сработала, и на сером ключ не срабатывает — полосы остались бы
  в спрайте. Резать надо по измеренной сетке, а не надеяться на цвет:
  у листа 2048x2048 перегородка вышла 18 пикселей ровно по центру, то есть
  клетки [0..1015] и [1033..2048].
* **Резать клетки РОВНО, одним размером, без подгонки.** Тогда совмещение,
  которое нарисовала модель, сохраняется точно: у готового листа все четыре
  кадра легли на одну линию земли (y=56–57 из 64) и в один центр по
  горизонтали. Именно это и нельзя доверять `--grid`.
* **Просьба про одну линию земли сработала.** Это самое хрупкое требование,
  и его стоит повторять в промпте дословно.
* **Модель всё равно рисует ТЕНЬ под фигурой**, хотя в базе стиля стоит «no
  cast shadow». Так вышло у Спороносца: под каждым кадром лежала фиолетовая
  полоса. Оставлять её нельзя — игра рисует свою тень (`drawShadow`), и
  получились бы две.

  Снимается точно, и вот почему: модель рисует тень как ЗАТЕМНЁННЫЙ ФОН, то
  есть по той же оси, что и ключ. У Спороносца полоса была RGB(157,12,153) —
  красный и синий равны, зелёного почти нет. Собственный фиолетовый существа
  (#6b2d5c) так не выглядит: у него R и B расходятся, а зелёного втрое
  больше. Отсюда правило, которым тень заменяется на чистую магенту до
  вырезания ключа:

  ```python
  shadow = (abs(R - B) <= 10) & (G <= 28) & (R >= 55)
  ```

  Проверять обязательно: сколько пикселей правило задело внутри самой фигуры.
  У Спороносца — 366 из двух с лишним миллионов, то есть ничего.

Порядок сборки целиком:

```
# 1. нарезать 2x2 по измеренной сетке и уложить в строку 4x1 (три строки на Python)
# 2. снять ключ — в смерти всегда есть светлые споры, значит pale
python3 tools/cut_key.py полоса.png вырезано.png --mode pale
# 3. ужать до 256x64 и положить в assets-raw/enemies/<враг>_death.png
# 4. node tools/normalize.mjs <враг>_death
```
