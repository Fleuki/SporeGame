...
при сборке листа, и это нормальная практика.

Что модель так и не смогла, сколько ни проси, — **настоящую прозрачность**.
Оба раза она РИСОВАЛА шахматку, то есть узор, которым редакторы показывают
прозрачный фон. Во второй раз файл был даже PNG, но альфа-канал в нём
полностью непрозрачный. Планируй, что фон придётся снимать заливкой по
цветности: фон ахроматичен, а плащ и склянки цветные.
