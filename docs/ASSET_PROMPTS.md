...
```
python3 tools/cut_key.py вход.png docs/assets/images/ui/emblem.png
python3 tools/cut_key.py вход.png docs/assets/images/effects/acid_pool.png --grid 4x1 --cell 256
```

Альфа считается не по расстоянию до ключа, а по тому, сколько фона вообще
можно вычесть из пикселя, не уведя цвет в минус. Расстояние даёт завышенную
альфу на мягком крае, и вокруг спрайта остаётся розовая кайма.
...
## 4. ЭМБЛЕМА ДЛЯ СТАРТОВОГО ЭКРАНА — ЖДЁТ ПЕРЕГЕНЕРАЦИИ

Рисунок отличный, но пришёл с НАРИСОВАННОЙ шахматкой (57% кадра), а у эмблемы
есть мягкое: свечение под шляпкой, лучи мицелия, блики на склянках. Ровно тот
случай, который постобработкой не чинится — см. пункт 2 в начале файла.
Нужен повтор с магентой; промпт ниже уже с ней.
