...
## 4. ЭМБЛЕМА — ВЫРЕЗАНА, ЖДЁТ СТАРТОВОГО ЭКРАНА

`ui/emblem.png` (512x512) лежит в репозитории. Показывать её пока негде:
стартового экрана нет (ЭТАП 4 в `ROADMAP.md`), страница открывается сразу в
бой.
